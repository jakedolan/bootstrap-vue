...
const BVFormRatingStar = defineComponent({
    ...
    render() {
        ...
        return h(
            ...
            }, [h('span', { class: 'b-rating-icon' }, [this.normalizeSlot(type, slotScope)])]
        )
    }
})
...
export const BFormRating = /*#__PURE__*/ defineComponent({
    ...
    render() {
        ...
        if (showClear && !disabled && !readonly) {
            const $icon = h('span', { class: 'b-rating-icon' }, [
                ($slots[SLOT_NAME_ICON_CLEAR] || this.iconClearFn)()
            ])
            $content.push(
                h(
                    'span', {
                        class: ['b-rating-star b-rating-star-clear flex-grow-1', { focused: hasFocus && computedRating === 0 }],
                        tabindex: isInteractive ? '-1' : null,
                        onClick: () => this.onSelected(null),
                        key: 'clear'
                    }, [$icon]
                )
            )
        ...
        for (let index = 0; index < computedStars; index++) {
            ...
            $content.push(
                h(BVFormRatingStar, {
                    ...
                    scopedSlots: {
                        empty: $slots[SLOT_NAME_ICON_EMPTY] || this.iconEmptyFn,
                        half: $slots[SLOT_NAME_ICON_HALF] || this.iconHalfFn,
                        full: $slots[SLOT_NAME_ICON_FULL] || this.iconFullFn
                    },
                    key: index
                })
            )
        ...
        if (this.showValue) {
            $content.push(
                h(
                    'b', {
                        class: 'b-rating-value flex-grow-1',
                        'aria-hidden': 'true',
                        key: 'value'
                    },
