...
export const BAvatar = /*#__PURE__*/ defineComponent({
    ...
    render() {
        ...
        if (this.hasNormalizedSlot()) {
            ...
            $content = h('span', { class: 'b-avatar-custom' }, [this.normalizeSlot()])
        } else if (src) {
            $content = h('img', {
            ...
            $content = h(BIconPersonFill, { 'aria-hidden': 'true', alt })
        }
        ...
        const componentPropsData = button ? { variant, disabled, type } : link ? pluckProps(linkProps, this) : {}
        const componentOnData = button || link ? { onClick: this.onClick } : {}
        const componentData = {
            class: [CLASS_NAME, {
                // Apply size class
