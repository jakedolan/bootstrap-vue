...
export const paginationMixin = defineComponent({
    ...
    render() {
        ...
        const makeEllipsis = isLast => {
            return h(
                'li', {
                    class: ['page-item', 'disabled', 'bv-d-xs-down-none', fill ? 'flex-fill' : '', this.ellipsisClass],
                    role: 'separator',
        ...
        const makePageButton = (page, idx) => {
            ...
            const attrs = {
                ...
                'aria-label': hasPropFunction(labelPage) ?
                    /* istanbul ignore next */
                    labelPage(pageNumber) : `${isFunction(labelPage) ? labelPage() : labelPage} ${pageNumber}`,
        ...
        const pageOnContent = isNav ? {} : { keydown: this.handleKeyNav }

        const $pagination = h(
            'ul', {
                class: ['pagination', 'b-pagination', this.btnSize, this.alignment, this.styleClass],
